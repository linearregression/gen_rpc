...
-export([supervisor_black_box/1,
        ...
        block_call_with_receive_timeout/1,
        call/1,
        call_mfa_undef/1,
        call_mfa_exit/1,
        call_mfa_throw/1,
...
block_call_mfa_undef(_Config) ->
    ...
    SendTO = 200,
    {badrpc, {'EXIT', {undef,[{os,timestamp_undef,_,_},_]}}} = gen_rpc:block_call(?SLAVE, os, timestamp_undef, SendTO),
...
block_call_mfa_exit(_Config) ->
    ...
    SendTO = 200,
    {badrpc, {'EXIT', die}} = gen_rpc:block_call(?SLAVE, erlang, exit, ['die'], SendTO),
...
block_call_mfa_throw(_Config) ->
    ...
    SendTO = 200,
    'throwXdown' = gen_rpc:block_call(?SLAVE, erlang, throw, ['throwXdown'], SendTO),
    ok = ct:pal("Result [block_call_mfa_undef]: signal=EXIT Reason={die}").

block_call_with_receive_timeout(_Config) ->
    ok = ct:pal("Testing [block_call_with_receive_timeout]"),
    SendTO = 1,
    {badrpc, timeout} = gen_rpc:block_call(?SLAVE, timer, sleep, [500], SendTO),
    ok = timer:sleep(500).
